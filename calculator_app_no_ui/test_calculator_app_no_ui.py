...
from calculator_app_no_ui import app
...
def test_calculate_route():
    with patch("calculator_app_no_ui.calculate_rpn") as mock_calculate_rpn, patch(
        "calculator_app_no_ui.insert_into_db"
    ) as mock_insert_into_db:
        # Définir la valeur de retour de la fonction simulée
        mock_calculate_rpn.return_value = 4

        response = client.post("/calculate", params={"text": "2 2 +"})
        ...
        assert response.status_code == 200
        assert response.json() == {"operation": "2 2 +", "result": 4}
...
def test_calculate_route_invalid(input_data, expected_error):
    response = client.post("/calculate", params={"text": input_data})
...
def test_view_data_route():
    ...
    with patch("calculator_app_no_ui.view_database", return_value=mock_data):
